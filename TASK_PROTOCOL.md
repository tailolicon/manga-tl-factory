...
## Coordinator-less test mode

`STANDALONE_TEST.md` permits exactly one special pre-coordinator task class: bootstrap of an explicitly selected `pending_bootstrap` request. Its deterministic test envelope is generated by `manga_factory.standalone`. Any downstream `task_proposals` remain proposals only; standalone workers do not create or run child tasks.
